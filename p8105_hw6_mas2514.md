...
For comparison, the code below fits a main-effects model including only
birth length and gestational age as predictors.
...
```
    ...
    ## 2 birth_length       129.       1.99      64.6 0       
    ## 3 gestational_age     27.0      1.72      15.7 2.36e-54

For comparison, the code below fits an interaction model including head
circumference, birth length, sex, and all corresponding interaction
terms (including the three-way interaction).
...
```
    ...
    ##   term                                      estimate std.error statistic p.value
    ##   <chr>                                        <dbl>     <dbl>     <dbl>   <dbl>
    ## 1 (Intercept)                               -7.18e+3  1265.       -5.67  1.49e-8
    ## 2 birth_length                               1.02e+2    26.2       3.90  9.92e-5
    ## 3 head_circumference                         1.82e+2    38.1       4.78  1.84e-6
    ## 4 sexFemale                                  6.37e+3  1678.        3.80  1.47e-4
    ## 5 birth_length:head_circumference           -5.54e-1     0.780    -0.710 4.78e-1
    ## 6 birth_length:sexFemale                    -1.24e+2    35.1      -3.52  4.29e-4
    ## 7 head_circumference:sexFemale              -1.98e+2    51.1      -3.88  1.05e-4
    ## 8 birth_length:head_circumference:sexFemale  3.88e+0     1.06      3.67  2.45e-4

To compare the predictive performance of the three models, the code
below applies `crossv_mc` with 100 random training-test splits. For each
split, all three models were fit to the training data, and the root mean
squared error (RMSE) was computed on the corresponding test set.
...
The violin plot below summarizes the distribution of cross-validated
RMSE for each model across the 100 splits. This provides a direct
comparison of out-of-sample predictive accuracy and allows us to assess
whether the additional predictors or interaction terms meaningfully
improve performance relative to the simpler models.
...
<img src="p8105_hw6_mas2514_files/figure-gfm/unnamed-chunk-14-1.png" width="90%" />

Across the 100 cross-validation splits, the proposed model shows the
lowest and most stable prediction error, with RMSE values generally
centered around the mid-270s. The interaction model performs slightly
worse, with RMSEs clustered around the low- to mid-280s, indicating that
adding the three-way and two-way interactions between infant sex, head
circumference, and birth length does not meaningfully improve predictive
accuracy. The main-effects model using only birth length and gestational
age performs substantially worse than the other two models, with RMSE
values in the 320–350 range. Overall, the proposed model provides the
strongest out-of-sample predictive performance among the three,
suggesting that incorporating additional maternal and demographic
predictors improves prediction of birthweight beyond infant size and
gestational age alone, while the complex interaction structure does not
